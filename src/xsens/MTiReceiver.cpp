...
MTiReceiver::MTiReceiver(uintptr_t gpioMmapPtr, uintptr_t uartMmapPtr, uintptr_t dmaMmapPtr)
        ...
        uart_(std::make_unique<UARTDMAReader>(Get_UART<2>(uartMmapPtr), Get_DMA<3>(dmaMmapPtr), 2, gpioMmapPtr, 151, 460800)),
        mtiParser_(std::make_unique<MTiParser>())
{
    ...
    mtiParser_->AddCallback([&](const XsMessage& msg, auto info) {
        ValidMessageCb(msg);
    });
