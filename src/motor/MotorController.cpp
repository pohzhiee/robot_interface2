...
template<typename T>
inline std::array<uint8_t, sizeof(T)> BytesFrom(T input)
{
...
{
    auto a = [this](auto* topicName, const auto& msg,
            auto time, auto clock, auto id) { OnCmdMsg(topicName, msg, time, clock); };
    motorCommandSub_->AddReceiveCallback(a);
}
void MotorController::OnCmdMsg(const char* topic_name_, const robot_interface::MotorCmdMsg& msg, long long int time_, long long int clock_)
{
    // Create message
    ...
    for (const auto& cmd: msg.commands()) {
        ...
        assert(index<12);
        motorCommandSingles.at(index) = MotorCommandSingle{
                .CommandType = ProtoMotorCmdToInterface(cmd.command()),
    ...
    auto hasFront = std::any_of(msg.commands().cbegin(), msg.commands().cend(), [](const auto& cmd) { return cmd.motor_id()<=5; });
